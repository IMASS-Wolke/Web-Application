...
function FasstRunner() {
  ...
  const [hasRun, setHasRun] = useState(false); // 🔹 new state flag

  const runFasst = async () => {
    setStatus("Running FASST model...");
    setResult(null);
    setHasRun(false); // reset before starting
    ...
    try {
      ...
      setHasRun(true); // ✅ show results once complete
    } catch (err) {
  ...
  return (
    <div className="fasst-runner">
      ...
      {/* Show results only after the model has been run */}
      {hasRun && result && (
